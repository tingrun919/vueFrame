  ...
  <div>
    <div v-if="searchable && searchPlace === 'top'" class="search-con search-con-top">
      <Select v-model="searchKey" class="search-col">
        <Option v-for="item in columns" v-if="item.key !== 'handle'" :key="`search-col-${item.key}`" :value="item.key">
{{ item.title }}
</Option>
      </Select>
    ...
    methods: {
      ...
      handleColumns(columns) {
        this.insideColumns = columns.map((item, index) => {
          let res = item;
          //修改为每个元素超出就缩略显示，使用tooltip展示 lzj 2018/10/18
          if (res.tooltip === undefined) {
            res.tooltip = true;
          }
